...
def fire_due_events(conn, brain: child_mod.ChildBrain, child_id: str,
    ...
    with tx(conn):
        rows = conn.execute(
            "SELECT * FROM scheduled_event WHERE child_id=? AND kind='night_cry'"
            " AND status='pending' AND due_at<=? ORDER BY due_at",
            (child_id, t)).fetchall()  # 只领夜哭:ask()等别族各有自己的 fire
        for ev in rows:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    observed = []
                # 摩擦轴节律(被晾/摔门/深夜彩蛋)+可见成长
                # (宝贝盒/小本子):全幂等;故障=空照旧(tick 不许炸)
                try:
                    from .friction import tick_friction
                    fric = tick_friction(conn, cid, now=t)
                except Exception:
                    fric = {}
                try:
                    from .visible_growth import tick_growth
                ...
                out = {"scheduled": scheduled, "fired": len(fired), "stolen": stolen,
                       "events": evs, "outbox": posted, "asks": asks,
                       "choices": choices, "chains": chains}
